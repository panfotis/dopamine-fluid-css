...
| `lib/generator-sass.js` | Auto-generates `_dopamine-functions.scss` alongside `.scss` output, parameterized by the user's config |
...
- **`_dopamine-functions.scss` is auto-generated** alongside `.scss` output. The standalone `addons/sass/_functions.scss` is a separate hand-authored file with default values baked in — consumers pick one based on whether they run the CLI or not.
...
- **`addons/sass/_functions.scss`** — hand-authored standalone Sass addon. Separate from the auto-generated `_dopamine-functions.scss`; both ship.
