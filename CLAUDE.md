...
- **Unit suffix `-N<unit>`** for sizing prefixes only (`w`, `h`, `maxw`, `minw`, `maxh`, `minh` — gated by `allowsUnits: true` in `PREFIX_MAP`). Units: `%`, `vw`, `vh`, `vmin`, `vmax`, `svw`, `svh`, `lvw`, `lvh`, `dvw`, `dvh`, `ch`. No fluid mode for units (`w-50%-80%` is rejected — semantically muddy).
...
- **`stack` is the only child-combinator prefix**: `childCombinator: true` in `PREFIX_MAP` makes the generator emit `:where(.stack-16-32) > * + * { margin-top: … }` instead of styling the class itself. The `:where()` is load-bearing — it zeroes specificity so any `mt-*` on a child overrides the stack regardless of source order. Don't remove it.
- **`cols-fit-N` / `cols-fill-N` are the RAM pattern**: `repeat(auto-fit|fill, minmax(min(Nrem, 100%), 1fr))`. Single minimum only — no max by design (`1fr` caps growth; a max would leave dead row space and force a justify-content decision). `diagnoseClass` catches `cols-fit-250-350` and must stay ahead of the unknown-breakpoint check, which would misread `fit` as a breakpoint. Breakpoint goes after the compound prefix: `cols-fit-md-300`.
...
## Components — notes for maintainers

Components are hand-authored (`addons/components/<name>/<name>.{scss,js}`), outside the generator's purview. Each file pair is independent — no shared runtime — and the SCSS build picks them up via the glob `addons/components:css/components`. Adding a new component = new directory + two files; no registry, no init.js edit.

**menu-drawer** has the most moving parts; load-bearing decisions:

- **Two modes, one markup.** Below `$menu-drawer-bp` (default 992px) `.menu-dropdown-0` becomes a fixed right-sliding drawer; above, the same panel is an absolute-positioned dropdown under its parent `<li>`. The JS drives `.menu-drawer--open` on the `<li>` for both modes (BEM-aligned with `.modal--open` etc.). Mobile-only machinery (injected back/title header, viewport sizing via `--menu-drawer-top` / `visualViewport`, focus trap, `role="dialog"` / `aria-modal`) is gated inside `if (isMobile())` blocks.
- **Trigger detection is class-structural, not attribute-based.** A trigger is any `<a>` or `<span>` direct child of `.menu-level-0 > .menu-item` whose `<li>` also contains a direct-child `.menu-dropdown-0`. No `data-toggle="dropdown"` is read — Bootstrap is not assumed. Three real Drupal markup samples (Kraft, HelleniQ, The Ellinikon) drove this shape; all tests should keep passing against those.
- **Click-to-toggle is the default on desktop; hover is opt-in** via `.menu-drawer-hover` on the root `<ul>`. Don't re-add `:hover`/`:focus-within` to the always-on rule — that was deliberately moved behind the modifier so touch devices on desktop viewports behave predictably.
