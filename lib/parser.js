...
// Value can be `n` prefixed to indicate a negative (e.g. `mt-n10` → -10)
const FIXED_PATTERN = /^(?<prefix>[a-z]+(?:[xy])?)-(?<value>n?\d+)$/;
...
const BP_PATTERN = /^(?<prefix>[a-z]+(?:[xy])?)-(?<bp>[a-z]+)-(?<minPx>n?\d+)(?:-(?<maxPx>n?\d+))?(?:--(?<vpMin>\d+)-(?<vpMax>\d+))?$/;

// Parse a signed value string: "10" → 10, "n10" → -10, "n0" → 0 (not -0)
function parseSignedInt(str) {
  const n = str[0] === 'n' ? -parseInt(str.slice(1), 10) : parseInt(str, 10);
  return n === 0 ? 0 : n;
}
...
function parseClass(cls, config) {
  ...
  if (bpMatch) {
    ...
    const min = parseSignedInt(minPx);
    const max = maxPx !== undefined ? parseSignedInt(maxPx) : null;
  ...
  if (baseMatch) {
    ...
    const min = parseSignedInt(minPx);
    const max = parseSignedInt(maxPx);
...
function diagnoseClass(cls, config) {
  ...
  if (invertedRange) {
    ...
    const min = parseSignedInt(minS);
    const max = parseSignedInt(maxS);
    if (min >= max) {
