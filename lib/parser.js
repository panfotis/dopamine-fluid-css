...
const { PREFIX_MAP, buildBreakpointViewportMap, resolveBreakpointViewport } = require('./config');
 ...
 *                         viewport auto-derived from breakpoint range (576->768)
 ...
 */
function resolveViewport(descriptor, config, bpViewportMap) {
  const { prefix, breakpoint, inlineVpMin, inlineVpMax } = descriptor;
  ...
  if (breakpoint) {
    const bpVp = resolveBreakpointViewport(breakpoint, bpViewportMap);
    if (bpVp) return { ...bpVp, source: 'breakpoint' };
  }
