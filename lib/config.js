 ...
 * up to the next breakpoint's min-width. For the last breakpoint,
 * Dopamine uses global viewport.max when possible, otherwise it
 * extrapolates one final range from the previous breakpoint delta.
 */
 ...
 *   vpMax = the NEXT breakpoint's px value, or a safe fallback for the last breakpoint
 */
/**
...
function buildBreakpointViewportMap(config) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (next) {
      vpMax = next[1];
    } else if (config.viewport.max > px) {
      vpMax = config.viewport.max;
    } else if (prev) {
      vpMax = px + (px - prev[1]);
    } else {
      vpMax = px + Math.max(config.viewport.max - config.viewport.min, 1);
    }

    map[name] = { vpMin: px, vpMax };
  }
