...
function build(input, opts, config) {
  ...
  for (const cls of allClasses) {
    ...
    const result = generateGridRule(gridDesc, config);
...
function buildOutput(baseRules, bpRules, config, opts, gridBase, gridBp) {
  ...
  // Breakpoint rules — sorted by breakpoint px value, grid then fluid per bp
  ...
  for (const [bpName, bpPx] of sortedBps) {
    ...
    if (opts.header !== false) blocks.push(`/* ${bpName} — min-width: ${bpPx}px */`);
    if (hasGrid)  blocks.push(...gridBucket);
    if (hasFluid) blocks.push(...fluidBucket.values());
  }
