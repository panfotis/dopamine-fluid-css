...
const { resolveFiles }                                = require('./scanner');
const { parseClass, resolveViewport, diagnoseClass }  = require('./parser');
const { collectClassCounts }                          = require('./counter');
...
const { generateSassFunctions }                       = require('./sass-generator');
...
function build(input, opts, config) {
  ...
  const t0 = Date.now();
  const timing = process.env.DOPAMINE_TIMING ? { scan: 0, parse: 0, generate: 0, manifest: 0 } : null;
  console.log(`\x1b[36m◆\x1b[0m  Scanning ${files.length} file${files.length > 1 ? 's' : ''}...`);

  const tScan = Date.now();
  const allClasses = collectClassCounts(files);
  if (timing) timing.scan = Date.now() - tScan;
  ...
  if (opts.classes) {
    ...
    if (fs.existsSync(classesPath)) {
      ...
      lines.forEach(c => {
        if (!allClasses.has(c)) allClasses.set(c, 0);
        classesFromFile.add(c);
      });
  ...
  for (const cls of allClasses.keys()) {
  ...
  if (timing) timing.parse = Date.now() - tParse;

  // Warn about unrecognized classes from the classes file — include a per-class
  // diagnostic where we can (only for file-sourced classes; template-extracted
  // tokens can be noisy and false-positive).
  if (classesFromFile.size > 0) {
    ...
    if (unmatched.length > 0) {
      console.warn(`\x1b[33m⚠\x1b[0m  Skipped ${unmatched.length} unrecognized class${unmatched.length !== 1 ? 'es' : ''}:`);
      for (const cls of unmatched) {
        const reason = diagnoseClass(cls, config);
        if (reason) {
          console.warn(`      \x1b[33m${cls}\x1b[0m — ${reason}`);
        } else {
          console.warn(`      \x1b[33m${cls}\x1b[0m`);
        }
      }
    }
  }
  ...
  if (timing) timing.generate = Date.now() - tGenerate;
  ...
  if (opts.manifest && !opts.dryRun) {
    ...
    for (const name of compiledNames) {
      compiledCounts.set(name, allClasses.get(name) || 0);
    }
    const tManifest = Date.now();
    writeManifest(opts.manifest, compiledCounts);
    if (timing) timing.manifest = Date.now() - tManifest;
  }

  printSummary(totalRules, stats, gridCount, files, config, Date.now() - t0);
  if (timing) printTiming(timing);
}

function printTiming(timing) {
  console.log(`\x1b[1m  Timing (ms)\x1b[0m`);
  console.log(`  ─────────────────────────────────`);
  console.log(`  scan ${timing.scan.toString().padStart(5)}   parse ${timing.parse.toString().padStart(5)}   generate ${timing.generate.toString().padStart(5)}   manifest ${timing.manifest.toString().padStart(3)}`);
  console.log('');
}
...
function printSummary(total, stats, gridCount, files, config, ms) {
...
/**
 * What chokidar should watch for a given input, which — like `resolveFiles` —
 * may be a file, a directory, or a glob. Only the directory case can use the
 * `cwd` + `**\/*.ext` form; pointing `cwd` at a file (or at a path that doesn't
 * exist, as a glob does) silently watches nothing.
 *
 * Returns `{ paths, options }` to spread into `chokidar.watch`.
 */
function watchTargets(input, ext) {
  const exts = (ext || 'twig,html,htm').split(',').map(e => e.trim().replace(/^\./, ''));

  if (fs.existsSync(input)) {
    const stat = fs.statSync(input);

    if (stat.isFile()) {
      return { paths: [path.resolve(input)], options: {} };
    }

    if (stat.isDirectory()) {
      return {
        paths: exts.map(e => `**/*.${e}`),
        options: { cwd: path.resolve(input) },
      };
    }
  }

  // Glob — chokidar matches it directly
  return { paths: input, options: {} };
}
...
async function run(input, opts) {
  ...
  if (opts.watch) {
    ...
    const { paths, options } = watchTargets(input, opts.ext);
    const watchCwd = options.cwd || process.cwd();

    console.log(`\x1b[36m◆\x1b[0m  Watching for changes... (Ctrl+C to stop)\n`);
    build(input, opts, config);

    chokidar.watch(paths, { ...options, ignoreInitial: true }).on('all', (event, filePath) => {
      const abs = path.resolve(watchCwd, filePath);
      console.log(`\x1b[90m${event}: ${path.relative(process.cwd(), abs)}\x1b[0m`);
...
module.exports = { run, watchTargets };
