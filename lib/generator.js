...
function generateRule(descriptor, viewport, _config, includeComment = true) {
  ...
  if (mode === 'grid') {
    if (descriptor.gridFit) {
      const { kind, minPx: fitMin } = descriptor.gridFit;
      const minRem = round(fitMin / 16, 4);
      value = `repeat(auto-${kind}, minmax(min(${minRem}rem, 100%), 1fr))`;
    } else {
      value = buildGridColumns(descriptor.gridValue);
    }
    commentStr = '';
  } else if (mode === 'ratio') {
    value      = descriptor.ratioValue;
    commentStr = '';
  } else if (mode === 'auto') {
    value      = 'auto';
    commentStr = '';
    ...
    if (unitless) {
      // order: "5"; lh: "1.5" (literal); span (via valuePrefix): "span 5"
  ...
  // childCombinator prefixes (stack) style the children, not the element
  // itself; :where() keeps specificity at zero so child utilities always win.
  const selectorStr = PREFIX_MAP[prefix].childCombinator
    ? `:where(.${selector}) > * + *`
    : `.${selector}`;
 ...
 *
 * A bare `Nfr` track is `minmax(auto, Nfr)`, so a child's intrinsic min-width
 * (long unbreakable word, wide image) can stretch its track and break the
 * ratio. Clamping the minimum to 0 keeps the tracks true to the ratio.
 */
function buildGridColumns(value) {
  if (value.includes('.')) {
    return value.split('.').map(n => `minmax(0, ${n}fr)`).join(' ');
  }
