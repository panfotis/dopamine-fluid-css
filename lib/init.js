...
const STARTER_FILES = [
  ...
  'dopamine-safelist.txt',
  'scss/main.scss',
  'scss/custom/README.md',
  'templates/index.html',
];

// Starter files where an existing differing copy is kept instead of erroring.
// For demo placeholders the consumer's own file always beats the starter's.
const OPTIONAL_STARTER_FILES = new Set([
  'templates/index.html',
]);
...
function buildInitMessage(result, options = {}) {
  ...
  if (result.files.keptExisting.length > 0) {
    lines.push(`  Kept existing       ${result.files.keptExisting.join(', ')}`);
  }
...
function planStarterWrites(targetDir, starterFiles, force) {
  ...
  for (const file of starterFiles) {
    ...
    if (!force) {
      if (OPTIONAL_STARTER_FILES.has(file.relativePath)) {
        keptExisting.push(file.relativePath);
        continue;
      }
      conflicts.push(file.relativePath);
      continue;
